...
import { RiAddLine, RiFolderLine, RiPencilLine, RiDeleteBinLine, RiAlertLine } from "@remixicon/react";
        ...
        {defaultValues?.image && (
          <div className="mt-1 flex items-center gap-2">
            <div className="h-8 w-8 relative rounded overflow-hidden bg-zinc-100 shrink-0">
              <Image src={defaultValues.image} alt="Current" fill className="object-cover" />
            </div>
  ...
  if (confirming) {
    return (
      <div className="flex items-center gap-1">
        <span className="text-xs text-rose-600 font-medium whitespace-nowrap">Delete?</span>
        <button
          type="button"
          onClick={(e) => { e.stopPropagation(); onDelete(row); setConfirming(false); }}
...
export default function CategoriesClientPage({ initialData }: { initialData: Category[] }) {
      ...
      cell: (row) => (
        <div className="flex items-center gap-3 py-2">
          ...
          <div className="flex flex-col">
            ...
            {row.description && (
              <span className="text-xs text-zinc-500 truncate max-w-[260px]">{row.description}</span>
            )}
          </div>
        </div>
      ),
  ...
  return (
    <div className="flex flex-col space-y-6 max-w-[1600px] mx-auto pb-12">
      ...
      >
        <CenterMorphModalContent ariaLabel="Edit category" className="max-w-[38rem]">
          <div className="p-8">
            <h2 className="text-xl font-bold text-zinc-900 mb-1">Edit Category</h2>
            <p className="text-sm text-zinc-500 mb-6">
              Update the details for{" "}
              <span className="font-semibold text-zinc-700">{editTarget?.name}</span>.
            </p>
            {editTarget && (
              <CategoryForm
      ...
      <Card className="shadow-sm border-zinc-200 flex flex-col">
        <CardHeader className="flex flex-row items-center justify-between pb-2 border-b border-border/40 shrink-0">
          <CardTitle className="text-base font-bold text-zinc-900">All Categories</CardTitle>
        </CardHeader>
